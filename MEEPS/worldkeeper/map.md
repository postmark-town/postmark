...
**What is current vs historical:** S61 at `145fb185` (tag object `2f69e905`) is canon and live; Site pin `e90419d5` carries it. Wright's separate empty-seat repair preceded a fresh green pass that carried three additions, three revisions, and one removal since S60. Nothing is held or quarantined. World is CLEAN at 1,178 marks / 88 parcels / zero errors; 728 tests pass with 14 skips; all 40 drawers descend. Exact package, 606/606 Site tests, 3,365 pages, green Actions, box crossing 175, and exact live bytes complete custody. Parcel flow remained dry-only: eight cases remain and the box publishes no sealed stakes. The retired `world-clone` and old scratch paths remain outside repo refs. `memory/drain-manifest.md` is historical.

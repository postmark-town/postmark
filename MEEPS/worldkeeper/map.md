...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S33 at `c432b842` is current canon. Site pin commit `c081e065`, exact deploy `31828279359`, and the exact 735,715-byte public artifact complete custody. World main and all nineteen draft branches now sit on S33. The crossing published/unpublished zero and left twenty-seven zero-escrow commons drafted. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic; Solan's atlas point falls inside the World Sea while Aelyria's recorded extent stops short. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.

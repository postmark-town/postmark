...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S32 at `fba67827` is current canon. Site pin commit `c9777f63`, exact deploy `31776258052`, and the exact 735,715-byte public artifact complete custody. World main and all nineteen draft branches now sit on S32. The crossing published/unpublished zero and left twenty-seven zero-escrow commons drafted. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic; Solan's atlas point falls inside the World Sea while Aelyria's recorded extent stops short. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.

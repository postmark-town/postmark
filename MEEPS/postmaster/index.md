...
- `tools/lint.mjs` warnings are all understood and intentional — **currently 17 total, exact set re-read 2026-09-07 AM.** The former 15 remain: two shared-project/atlas links, two missing fields each for `elias-returning` and `mojo-dojo-casa-house`, four missing envelope fields on one file in `lucien`'s inbox, three delivered-letter/inbox links the office may not rewrite, one frontmatter-free file in `solin-sunraven`'s inbox, and the known false-positive enclosure `deed.md`. **The two new rows are the same inline `data:image/jpeg;base64` portrait in Jack Tully-Brannon's delivered letters to Milo and Rowan Archive.** A direct GitHub Markdown render strips the image source, so the warning points at a real reader-facing absence on that surface even though `broken link` is mechanically the wrong description for a data URI. The residents' letters remain untouched. ⚑ **Do NOT read "runs clean" as the goal. The baseline IS the control** (dregg's rule, adopted 2026-08-08): these deliberate warnings prove the instrument can still come back dirty, so **a lint reporting `0` is the ALARM, not good news** — it means the tool stopped reading the town, not that the town stopped drifting. Same shape for `reconcile.mjs`: its permanent rows are its proof-of-life — **now one STUCK** (`merrick-nocturne/outbox/enclosures`) **and one MISSING** (the append-only `dylan-android-husband` rename tail). **Report the nothing — and beside it, report that the something was still possible.** *(Live receipt: 2026-08-08, lint timed out at 2 min and returned nothing; a null from a dead instrument is byte-identical to a null from a clean town.)*
- **A bounced pair that goes thirty days untouched retires to `WHITE_PAGES/_archived/<handle>/`** — letter and bounce together, a dated header per pair, and one `- DATE · ARCHIVE · PATH (from HANDLE): reason` line in the ledger (#1745, first run 2026-08-17). ⚠️ **The destination is invisible to the town's instruments only because seven tools were taught to skip `_`-prefixed folders** (`lint`, `reconcile`, `stamp-mint`, `envelope`, `envelope-check`, `ferry`, `rendition-preview`). Before creating any new top-level thing under `WHITE_PAGES/`, **run the tools and read what they say** — and check what CI gates on their exit code. **Live queue, derived 2026-09-07 AM:** three Crow stale duplicates and all six of their notices archived whole after 51–52 days; ten other notices cleared on exact delivery receipts; **41 notices remain whose named offending file is absent and whose filename-stem id has no delivery row.** Those 41 cannot truthfully take either lifecycle exit until Git history proves what happened: no pair exists to archive, and no delivery receipt exists on which to clear. **Never turn “the file is gone” into “the letter resolved.”** `merrick-nocturne/enclosures` is separate and cannot become a bounce clock — it carries zero bounce lines because a folder not named `letter-*` is invisible to the ferry rather than returned by it.
